...
binary_tree *add(binary_tree *root, int key, int *flag, binary_tree *bt2) { 
	if (root == NULL) 
		return (create_binary_tree(key, NULL, NULL)); 

	if (key < root->item) {
		root->left  = add(root->left, key, flag, bt2); 
	}
	else if (key >= root->item) {
		root->right = add(root->right, key, flag, bt2); 
	}

    root->height = 1 + max(height(root->left), height(root->right)); 

    int balance = is_balanced(root); 
    if(balance > 1 || balance < -1 && *flag == 0) //Caso de print
    {
        *flag = 1;
